# levain-calc — 사워도우 레시피 계산기 · 르방 변환기
...
```bash
npm install
npm run dev        # 개발 서버 — http://localhost:5173/levain-calc/
npm test           # 계산 로직 유닛 테스트 (Vitest)
npm run build      # 프로덕션 빌드 → dist/
npm run preview    # 빌드 결과 미리보기
```

> 개발 서버 주소에 `/levain-calc/` 경로가 붙는 것은 GitHub Pages 배포용 `base` 설정 때문입니다.

## GitHub Pages 배포

1. GitHub에 **`levain-calc`** 이름으로 저장소를 만들고 이 프로젝트를 push 합니다.
...
4. 배포 주소: `https://<사용자명>.github.io/levain-calc/`
