...
export function PrintFiche({ name, input, stats, pieces, precision, basis = 'total' }: Props) {
    ...
    <section className="print-only">
      <div className="mb-4 border-b-2 border-black pb-3">
        ...
        <p className="mt-1 text-sm tabular-nums">
          {date} · 총 수분율 {fmtPct(stats.hydrationPct)} · 소금{' '}
          {fmtPct(rebasePct(stats, stats.saltPct, basis))} · PFF {fmtPct(stats.pffPct)} · 총 반죽{' '}
          {fmtGrams(stats.doughWeight, precision)} g
          {pieces && pieces > 0
            ? ` · 분할 ${pieces} × ${fmtGrams(stats.doughWeight / pieces, precision)} g`
            : ''}
        </p>
      </div>
      <BakersTable input={input} stats={stats} precision={precision} pieces={pieces} basis={basis} />
    </section>
  );
}
